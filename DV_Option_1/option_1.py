...
def generate_design_option_one(df, length):
    ...
    fig = px.scatter(x=length, y=df['light'], color=df["productivity"], error_y=df['std_light'], labels=dict(x="Session Number", y="Light Values (Lumens)", color="Producitivity Rating"))
    ...
    fig = px.scatter(x=length, y=df['humidity'], color=df["productivity"], error_y=df['std_humidity'], labels=dict(x="Session Number", y="Humidity Values (%)", color="Producitivity Rating"))
