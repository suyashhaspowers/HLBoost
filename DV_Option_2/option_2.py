...
def generate_design_option_two(df, length):
    ...
    fig = px.scatter(x=length, y=df['light'], size=df["productivity"], error_y=df['std_light'], labels=dict(x="Session Number", y="Light Values (Lumens)", size="Producitivity Rating"))
    ...
    fig = px.scatter(x=length, y=df['humidity'], size=df["productivity"], error_y=df['std_humidity'], labels=dict(x="Session Number", y="Humidity Values (%)", size="Producitivity Rating"))
